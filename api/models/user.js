...
class User {
  ...
  async doIExist(email, password) {
    const user = await (
      await db.query(
        `SELECT LOWER(u.email), u.password, u.id_user FROM projetWeb.users u WHERE LOWER(u.email) = $1`,
        [email.toLowerCase()],
      )
    ).rows;
    if (user.length === 0) {
  ...
  async getSeller(id) {
    const seller = await (
      await db.query(
        `SELECT u.*, s.* FROM projetWeb.users u, projetWeb.seller s WHERE u.id_user = s.id_user AND s.id_user = $1`,
        [id],
      )
    ).rows;
    return seller[0];
